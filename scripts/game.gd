...
@onready var label_pongs = $Pontos
...
func spawn_asteroid():
	if (ball.started):
		...
		if(spawn_position != last_position):
			var asterioid_instance = asteroid_scene.instantiate()
			asterioid_instance.global_position = spawn_position.position
			if new_asteroid_color != null:
				asterioid_instance.get_node("Sprite2D").texture = new_asteroid_color
			add_child(asterioid_instance)
